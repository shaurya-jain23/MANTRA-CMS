                  ...
                  {permissions.map((perm) => (
                    <tr key={perm.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-500">{perm.resource.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end gap-4">
                          <button onClick={() => handleOpenForm(perm)} className="text-blue-600 hover:text-blue-900">
                            <Edit size={18} />
                          </button>
                          <button onClick={() => handleDelete(perm)} className="text-red-600 hover:text-red-900">
                            <Trash2 size={18} />
                          </button>
                        </div>
                      </td>
...
function PermissionManager() {
  ...
  const { register, handleSubmit, control, watch, formState: { errors } } = formMethods;
  ...
  const handleCreateDefaults = () => {
    showModal({
      ...
      message: 'This will create a standard set of permissions. Existing permissions with the same ID will be overwritten.',
  ...
  useEffect(()=> {
        const subscription = watch((value, {name})=> {
            if(name === 'category'){
              setResources([...PERMISSION_CATEGORIES.find(cat => cat.categoryId === value.category)?.resources] || [])
            }
        })
        return () =>{
    ...
    <div className="space-y-6">
      <div className="w-full flex flex-col md:flex-row justify-between mb-10">
        ...
        <div className="mt-6 flex justify-end gap-4">
          <Button onClick={handleCreateDefaults} variant="secondary" className="!rounded-lg !w-fit">
            Create Defaults
          </Button>
      ...
      {permissions.length === 0 && (
        <div className="text-center py-12 bg-white rounded-lg shadow border">
          ...
          <p className="text-gray-400 text-sm mt-2">Create your first permission or generate defaults.</p>
        </div>
      )}

      <ModalContainer isOpen={isPermissionFormOpen} onClose={closePermissionForm} className="max-w-2xl !p-0">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Controller
              ...
              rules={{
                ...
                validate: (value) => value !== '--Category--' || 'Please select permission category',
              }}
              render={({ field }) => (
                <Select
                  label="Category"
                  ...
                  options={PERMISSION_CATEGORIES.map(cat => ({ value: cat.categoryId, name: cat.categoryName?.replace(/\b\w/g, l => l.toUpperCase()) }))}
            ...
            <Controller
              ...
              rules={{
                ...
                validate: value => value !== '--Resource--' || 'Please select permission resource'
              }}
              render={({ field }) => (
                <Select
                  label="Resource"
            ...
            <Controller
              ...
              render={({ field }) => (
                <Select
                  ...
                  placeholder="--Action--"
                  {...field}
                  options={actionOptions.map(act => ({ value: act, name: act.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) }))}
          ...
          <div className="flex justify-end space-x-4 pt-4">
            <Button type="button" variant="secondary" onClick={closePermissionForm} disabled={isSubmitting} className="!w-full">
              Cancel
            </Button>
